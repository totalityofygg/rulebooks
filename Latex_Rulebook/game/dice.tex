...
\callout {
    \mybullet {
        \item \myredbold{\myanchor{Bump}{dice-bump}}: "Bumping" a roll means adding additional modifiers to the result. For example, you can Bump a \VIG try by rolling your \PRE (see the section on \mylink{Personality}{adventurer-personality} for more info).
        \item \myredbold{\myanchor{Failure}{dice-failure}}:  If you roll a \mybold{Natural} 1 or 2 on a die, it's a Failure. The consequences of Failure will be listed in the description (otherwise, there's no effect).
        \item \myredbold{\myanchor{Natural}{dice-natural}}: The number appearing on the die after the roll, without modification.
        \item \myredbold{\myanchor{Nudge}{dice-nudge}}: A \mybold{Nudged} die is moved to a different adjacent face. For example, if you rolled a d6 and got a 6, you could move the die to a 2,3,4,or 5 - but not a 1. Only certain powerful effects allow you to \mybold{Nudge} a roll.
        \item \myredbold{\myanchor{Restore(d)}{dice-restore}}: a \mybold{Spent} die that can now be used.
        \item \myredbold{\myanchor{Spend / Spent}{dice-spent}}: if you \mybold{Spend} a die, or if a die is \mybold{Spent}, it can't be used anymore. 
        \item \myredbold{\myanchor{Try}{dice-try}}: Synonym for roll i.e. if you were to make a "d6 try", you would roll a d6. There are 3 basic types of Tries in the ToY, described below.
    }
}
...
\begin{center}
\myredbold{A Failure is a Natural 1 or 2 on a die}
...
Power Dice are one or more dice of the same type. If you roll a Failure with a Power Die, it becomes \mylink{Spent}{dice-spent}. \mylink{Blood}{cruces-blood} and \mylink{Faith}{cruces-faith} are Power dice.

\example{Randy the Magnificent has 4 \mylink{Blood Dice}{cruces-blood}, and decides to use 3 of them to perform the \mylink{Secret of Charm}{secret-charm}. He rolls a 1, 2, and a 4. He resolves the effects of the spell, then puts the two dice he rolled Failures on to the side. He'll get these back after he rests - in the meantime, he has 2 Blood Dice to use.}
...
Duration Dice are used to measure how long an effect lasts during \mylink{Combat}{combat}. You'll see a Duration Die written like \DUR{d4}, which tells you which die the duration starts on. At the \mylink{Bottom of the Moment}{time-combat}, make a try:
...
Duration and time obviously go hand-in-hand; see the next section on \mylink{Time}{game-time} for more information.

Some \mylink{Arcana}{arcana} will have a \Duration associated with them; the dice type depends on the number of \DICE invested in the invocation of the Arcana:
...
Resource Dice are used to keep track of how many uses a consumable has left. The consumable may be physical (\mylink{Gear}{gear-equipment}, \mylink{Fetishes}{fetishes}, etc.)\ or abstract (\mylink{Personality}{adventurer-personality}, \mylink{Grace}{vulgate-sacraments}, etc.). 

If you roll a Failure with any kind of Resource Die, the Fallout moves the die \DCDOWN. If you roll a d2, the Resource Dice is \mylink{Spent}{dice-spent}.

Certain Resource Dice might have a \MAX value. For example, the Resource Die that represents your \mylink{Armor}{gear-armor} might be a d8 \MAX, but you've taken a few hits and its \UD is currently at a d4.  Unless otherwise specified, the Fallout affects the current and not the \MAX value i.e. "your \PRE moves \DCDOWN" vs. "your \MAX: \PRE moves \DCDOWN".
...
If you want to combine or split Resource Dice, you can do so as long as:
  ...
  \mybullet {
    \item the die to be combined or split are some kind of \mylink{Gear}{gear};
...
In a \mybold{Roll Twenty} try, you're attempting to roll a \mybold{20 or better} with a combination of dice, modifiers, and "\mylink{bumping}{dice-bump}". Think of it as "building a 20" using as many dice rolls and modifiers as you're allowed. A 19 fails, a 20 succeeds.
...
If an Adventurer is making a \RB try against a \mylink{Monster}{monsters}, the Monster adds its \HD as a modifier to its roll i.e. a 6 \HD Monster adds a +6 to its roll. Ties go to the Allies. If there's a tie between Allies, roll a d6 and add its result to your \RB. Keep going until there's a winner.
